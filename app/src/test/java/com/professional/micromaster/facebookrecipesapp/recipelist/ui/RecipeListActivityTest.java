...
import com.professional.micromaster.facebookrecipesapp.support.ShadowRecyclerView;
...
public class RecipeListActivityTest extends BaseTest {
    ...
    private void setUpShadowAdapter(int positionToClick) throws Exception {
        when(recipe.getSourceURL()).thenReturn("http://galileo.edu");
        when(recipeList.get(positionToClick)).thenReturn(recipe);

        RecyclerView recyclerView = (RecyclerView)activity.findViewById(R.id.recyclerView);
        RecipesAdapter adapterPopular = new RecipesAdapter(recipeList, imageLoader, onItemClickListener);
